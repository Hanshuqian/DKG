...
class MobileNetV2(nn.Module):
    def __init__(self, output_stride=8, BatchNorm=None, width_mult=1., pretrained=True):
        ...
        self.low_level_features = self.features[0:4] #低级特征：保留了更多的孔家细节和纹理特征，有助于分割精确地位对象的边界和小物体
        self.high_level_features = self.features[4:] #高级特征：提供了更抽象的语义信息，帮助模型理解图像中的整体结构和对象，有助于区分不同类别的物体，尤其是负责的背景中

    def forward(self, x):
        low_level_feat = self.low_level_features(x)
        x = self.high_level_features(low_level_feat) #在低级特征的基础上进一步提取高级特征
