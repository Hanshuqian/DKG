...
if __name__ == '__main__':
    ...
    for epoch_num in tqdm.tqdm(range(20), ncols=70):
        ...
        for batch_idx, (sample) in enumerate(train_loader):
            ...
            distance_0_obj = F.interpolate(distance_0_obj, size=data.size()[2:], mode='nearest') # torch.Size([8, 1, 512, 512])
            ...
            distance_obj = torch.cat((distance_0_obj, distance_1_obj), dim=1) # torch.Size([8, 2, 512, 512])
            ...
            low_uncertainty_front_map = low_uncertainty_float * pseudo_label # torch.Size([8, 2, 512, 512])
            ...
            # print("kl_div_obj:", kl_div_obj.item())
            ...
            print(torch.log(1 - prediction[:, 0, ...][(prediction[:, 0, ...] < args.pseudo_label_threshold) * (prediction[:, 0, ...] > lower_bound)]).shape)
            print(not_cup_loss_sum_batch.shape)
